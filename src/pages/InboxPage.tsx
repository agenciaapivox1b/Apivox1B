        ...
        <div className="p-4 border-b border-border">
          ...
          <div className="flex gap-1 flex-wrap">
            {statusFilters.map((f) => (
              ...
              >
                {f === 'all' ? 'All' : f === 'waiting_human' ? 'Waiting' : f.charAt(0).toUpperCase() + f.slice(1)}
              </button>
            ))}
          </div>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className={`text-[10px] px-1.5 py-0 ${statusBadge(conv.status)}`}>
                  {conv.status === 'waiting_human' ? 'waiting' : conv.status}
                </Badge>
          ...
          <>
            ...
            <div className="p-4 border-b border-border flex items-center justify-between bg-card">
              ...
              <div className="flex gap-1">
                <Button variant="ghost" size="sm" className="gap-1 text-xs"><UserCheck className="h-3.5 w-3.5" /> Assign</Button>
                <Button variant="ghost" size="sm" className="gap-1 text-xs"><CheckCircle className="h-3.5 w-3.5" /> Resolve</Button>
                <Button variant="ghost" size="sm" className="gap-1 text-xs"><StickyNote className="h-3.5 w-3.5" /> Note</Button>
